...
| `len_per_seq` / `sequence_length` | **1024 / 2048 / 4192** | Active tokens per sequence; `max_seq_len` matches |
...
| ----------------- | ----- | ----- | ----- | --------- |
| 1024              | 28.92 | 0.11  | 17.35 | **46.38** |
| 2048              | 37.89 | 0.10  | 18.28 | **56.27** |
| 4192              | 57.38 | 0.11  | 22.38 | **79.87** |


![L1 step totals vs sequence_length](breakdown_1/profiler_result/plot/plot/step_flow_bs8.png)

**Figure 1: L1 — accumulated NVTX time per step (`batch_size=8`).** Step1 (offload path) dominates end-to-end time and grows ~2× from 1024→4192 (28.9→57.4 ms); step2 evict is negligible (~0.1 ms); step3 onboard is second but smaller than step1 (17–22 ms). Total cold 3-step pass: **46–80 ms** depending on `sequence_length`.
...
| op                     | 1024      | 2048      | 4192      |
| ---------------------- | --------- | --------- | --------- |
| step1.lookup           | 3.74      | 3.86      | 4.13      |
| step1.allocate         | 1.11      | 1.12      | 1.10      |
| step1.offload_launch   | 10.60     | 9.34      | 10.30     |
| **step1.offload_wait** | **13.48** | **23.57** | **41.86** |
| step2.evict_gpu        | 0.11      | 0.10      | 0.11      |
| step3.lookup           | 3.81      | 3.63      | 3.50      |
| step3.allocate         | 0.47      | 0.49      | 0.47      |
| step3.onboard_launch   | 3.12      | 2.57      | 2.41      |
| step3.onboard_wait     | 9.95      | 11.60     | 16.01     |


![L2 nine step_ops vs sequence_length](breakdown_1/profiler_result/plot/plot/step_op_flow_bs8.png)

**Figure 2: L2 — nine fixed step_ops.** Within step1, **`offload_wait`** is the largest op (13.5→41.9 ms) and drives L1 growth; `offload_launch` stays ~9–11 ms; lookup ~3.7–4.1 ms. Step3 **`onboard_wait`** rises with length (10→16 ms) but stays below step1 wait.
...
| Call            | 1024     | 2048     | 4192     |
| --------------- | -------- | -------- | -------- |
| gpu.allocate_py | 1.91     | 1.96     | 1.92     |
| **gpu.put**     | **8.59** | **8.48** | **8.87** |
...
| Call                        | 1024 | 2048 | 4192 |
| --------------------------- | ---- | ---- | ---- |
| gpu.lookup_py               | 0.08 | 0.09 | 0.10 |
| flexkv.build_index_meta     | 0.33 | 0.37 | 0.39 |
| flexkv.client.get_match     | 2.41 | 2.47 | 2.65 |
| recsys.merge_lookup_results | 0.38 | 0.38 | 0.39 |
...
| Call                                     | 1024  | 2048  | 4192  |
| ---------------------------------------- | ----- | ----- | ----- |
| flexkv.client.try_wait                   | 0.22  | 0.16  | 0.18  |
| flexkv.finish_task (incl. `client.wait`) | 12.97 | 23.14 | 41.46 |
| gpu.release_offload_pages_py             | 0.21  | 0.21  | 0.15  |


![L3 function-level breakdown](breakdown_1/profiler_result/plot/plot/L3_breakdown_bs8.png)

**Figure 3: L3 — per-op function timeline (call order).** Under `offload_wait`, **`flexkv.finish_task` / `client.wait`** (~13–41 ms) accounts for almost all step1 wait; `client.try_wait` poll leaves are ~0.2 ms (not the same as one `offload_try_wait()` wall — see §1.10). **`gpu.put`** (~8.5–8.9 ms) is flat vs `sequence_length`; lookup cost is mostly **`flexkv.client.get_match`** (~2.4–2.7 ms).
...
| ----------------- | ---------------------- | ------------------------- | -------------- | -------- |
| 1024              | 6058.0                 | 5858.1                    | 714.3          | 85.5     |
| 2048              | 6085.7                 | 5884.5                    | 1430.0         | 89.0     |
| 4192              | 6096.7                 | 5895.6                    | 2960.5         | 89.0     |


![Init phase breakdown](breakdown_1/profiler_result/plot/plot/init_breakdown_bs8.png)

**Figure 4: Init phase (excluded from L1–L3 pipeline).** One-time **`create_kvcache_manager` + `register_gpu_cache_tables`** ~11.9 s dominates; **`prepare_inputs`** grows with `max_seq_len` (0.7→3.0 s). Not on the steady-state hot path after warmup.
...
step1.offload_wait                         ← L2: whole step (may call offload_try_wait N times)
  └ offload_try_wait()  × N              ← sub-exp: ONE call only (N=1 typical @bs8, see nsys Count)
       ...
       ├ finish_task → client.wait(completely=True)  (~13–41 ms, dominant)
...
| ------------- | ---------------------------------- | ----------------------------- | ------------------------- |
| 1024          | **15.35**                          | 0.12                          | 14.99                     |
| 2048          | **23.57**                          | 0.16                          | 23.08                     |
| 4192          | **41.86**                          | 0.18                          | 41.41                     |
...
1. **Bottleneck:** `step1.offload_wait`, ~3.1× from 13.5 ms → 41.9 ms as `sequence_length` grows.
